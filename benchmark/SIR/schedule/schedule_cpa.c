...
//substitution for fscanf
int my_fscanf_float(float* ratio){
...
test_main(void)  // "test_main"
{
    ...
    argv[1] = __VERIFIER_nondet_int();
    argv[2] = __VERIFIER_nondet_int();
    argv[3] = __VERIFIER_nondet_int();
    ...
    {
	init_prio_queue(prio, argv[prio]); //start from argv[3] to argv[1]
    }
    for (status = my_fscanf_int(&command);
	 ((status!=EOF) && status);
	 status = my_fscanf_int(&command))
    {
	switch(command)
	{
	case FINISH:
