...
//#define	MAXLINE	256
#define	MAXLINE	10  //modified by Ting Su


#ifndef MAXTBL
//#define	MAXTBL	1000
#define MAXTBL 20   //modified by Ting Su
#endif
...
int my_sscanf(int* r, int* c){

   if( line[0] >= '1' && line[0]< '9' && line[1] >='1' && line[1]<='9'){
   	*r = line[0] - 48;
   	*c = line[1] - 48;
	return 2;
   }
...
test_main(void ) //"test_main"
	...
	//while ( fgets( line, MAXLINE, stdin ) != NULL )	/* start new table */
	  while (my_fgets()!=NULL)
		{
		for ( p = line; *p != '\0' && my_isspace( (int)*p ); ++p )
			;
