...
#define MAX_FILE_SIZE 5  //max file size
//char file_content[MAX_FILE_SIZE]={'a', 'n', 'd', ' ', '\n'}; //input content from a file, created by Ting Su
...
//extern functions for cpachecker
...
//added by Ting Su, borrowed from CBMC: cbmc/src/ansi-c/library/string.c
...
test_main(void)  //"test_main"
{
	...
	file_content[i] = __VERIFIER_nondet_char();
      ...
      if(argc>2)
      {
          ...
          //exit(1); //change all exit(1) to exit() due to CPAchecker cannot recognize exit(1)
          exit();
      }
      stream_ptr=open_token_stream("myfile");
...
string FILENAME;
{
      ...
      //         exit(0);
      //     }
      stream_ptr->fp = file_content; //modified by Ting Su, assign the input array to fp
      return(stream_ptr);
}
...
//the mock function for fgets
char* my_fgets(char* str, int n, char* input_stream){

   int i=0;

   if(file_fd_index >= MAX_FILE_SIZE) //the end of the file
	return NULL;
   if(input_stream[file_fd_index] == '\n' || input_stream[file_fd_index] == '\0')
	return NULL;

   for(i=0; i<MAX_FILE_SIZE && i<n ; i++){
	str[i] = input_stream[i];
	//printf("%c", str[i]);
	file_fd_index ++;
   }
   //printf("----\n");
...
character_stream stream_ptr;
{
      ...
      {
              if(my_fgets(stream_ptr->stream+START,10-START,stream_ptr->fp) == NULL)/* Fix bug: add -START - hf*/
...
{
      char token_str[10]; /* This buffer stores the current token */
...
char ch;
{
      if(!my_isalpha(ch) && !my_isdigit(ch)) /* Check for digit and alpha */
          return(TRUE);
      return(FALSE);
}
